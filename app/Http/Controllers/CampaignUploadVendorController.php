...
class CampaignUploadVendorController extends Controller
{

public function campaign_creatives_vendor_list(Request $request)
{
...
return view('/admin/campaign_creatives_vendor_list',
['vendor_c'=>$vendor_c]);
}


public function getcampaigncreativesvendor(Request $request)
{
  ...
  // Total records
    ...
    ->where('tbl_mibl_campaign_creatives.vendor_id',$vendor_idd)
  ...
  // Total records
  $totalRecords = Campaign_Creatives::select('count(*) as allcount')->where('tbl_mibl_campaign_creatives.vendor_id',$vendor_idd)->count();
  ...
  $result_Filter->where('tbl_mibl_campaign_creatives.vendor_id',$vendor_idd);
  ...
  $result->where('tbl_mibl_campaign_creatives.vendor_id',$vendor_idd);
    if (!empty($vendor_name)) {
    $arr_2=explode(",",$vendor_name);
    $vendor_type_id=$arr_2[0];
    $vendor_id=$arr_2[1];
  ...
  //echo count($result);
    ...
    foreach($records as $record){
      ...
      if(!empty($record->id))
       {
        $APP_URL=$_ENV['APP_URL']."vendor-edit-campaign-creatives/".base64_encode($record->id);
...
public function add_campaign_creatives_vendor(Request $request)
{
...
//   ->where('flag',0)
  ...
  return view('/admin/add_campaign_creatives_vendor', ['vendor' => $vendor,'vendor_type'=>$vendor_type]);
}


public function insert_campaign_creatives_vendor(Request $request)
{
...
return redirect('vendor-add-campaign-creatives');   
}
...
public function edit_campaign_creatives_vendor($id)
{
...
return view('/admin/edit_campaign_creatives_vendor', ['vendor'=>$vendor,'vendor_type'=>$vendor_type,'edit_services' => $data]);
}


public function update_campaign_creatives_vendor(Request $request)
{ 
...
{
session()->flash('failmsg', 'Upload only .zip file');
return redirect('vendor-campaign-creatives-list');   
}
$campaign_name1=str_replace(" ","-",$campaign_name);
$campaign_name2=str_replace("/","-",$campaign_name1);
