      ...
      <div class="tab-content">
         <div id="login">
            <div class="wrapper">
               <div class="container">
                   @foreach($edit_services as $creatives)
                  <form method="POST" action="{{ url('/update_creative_vendor') }}" enctype="multipart/form-data">
                     {{ csrf_field() }}
                     <div class="row">
                     ...
                     <input type="text" class="form-control" id="advertisement_id"
                           placeholder="Advertisement ID" name="advertisement_id" value="{{$creatives->advertisement_id}}" readonly>
                     ...
                     <div class="row">
                        ...
                        <select id="language_id" name="language_id" class="form-control" required disabled>
                        ...
                        <label for="brand_id">Brand</label>
                           <select id="brand_id" name="brand_id" class="form-control" required disabled>
                     ...
                     <label for="archive_category_id">Archive Category</label>
                        <select id="archive_category_id" name="archive_category_id" class="form-control" required disabled>
                     ...
                     <div class="row">
                        ...
                        <label for="department_type_id">Department</label>
                           <select id="department_type_ids" name="department_type_id" class="form-control" required disabled>
                           ...
                           <script type="text/javascript">
                            document.getElementById('department_type_ids').value="{{$creatives->department_type_id}},{{$creatives->department_id}}";
                            </script>
                        </div>

                        <div class="form-group col-lg-6">
                        <label for="vendor_type_id">Vendor Name</label>
                           <select id="vendor_type_id" name="vendor_type_id" class="form-control"  required disabled>
                           ...
                           <?php } ?>
                           </select><?php  $name=session('name');?>
                           <!-- <input type="text" id="vendor_name" name="vendor_name" class="form-control" readonly value="{{$name}}"> -->
                           <script type="text/javascript">
                   ...
                   <!--
                     <div class="form-row">
                        ...
                        <div class="row">
                           ...
                           <div class="form-group col-lg-6">
                              <label for="type_id" class="ml-2">Type</label>
                              <input type="text" class="form-control" id="type_id" name="type_id" readonly value="{{$creatives->type_id}}">
                           </div>
                     ...
                     <div class="row" >
                        ...
                        <input type="text" class="form-control" id="remark" name="remark" required value="{{$creatives->remark}}">
                        </div>
                     </div>
                     <?php $user_login_type=session('user_login_type'); ?>
                     <input type="hidden" class="form-control" id="id" name="id" value="{{$creatives->id}}">
                     <input type="hidden" class="form-control" id="vendor_id" name="vendor_id" value="{{$creatives->vendor_id}}">
                     <input type="hidden" class="form-control" id="created_date" name="created_date" value="{{$creatives->created_date}}">
                     
                        
                    <input type="hidden" id="ids_1" name="ids_1" value="{{$ids_1}}">
                    <input type="hidden" id="ids_2" name="ids_2" value="{{$ids_2}}">
                    <input type="hidden" id="ids_3" name="ids_3" value="{{$ids_3}}">
                    <input type="hidden" id="ids_4" name="ids_4" value="{{$ids_4}}">
                    <input type="hidden" id="ids_5" name="ids_5" value="{{$ids_5}}">
                    <input type="hidden" id="ids_6" name="ids_6" value="{{$ids_6}}">
                    <input type="hidden" id="ids_7" name="ids_7" value="{{$ids_7}}">
                    <input type="hidden" id="ids_8" name="ids_8" value="{{$ids_8}}">
                     
                     <button class="btn upload-btn" type="submit">Update</button>
                     @if($user_login_type == 'Employee')
                     <button type="button"  class="btn upload-btn" onclick="getids()" >Approve</button>
                     <button type="button"  class="btn upload-btn" onclick="getids1()">Approve & Share Link</button>
                     @endif
                  </form>
                  <br>
...
<label class="ml-2" style="display:none;" id="old_preview">Old File Preview</label>
 ...
 <!-- @if($creatives->file_type == 'image')   
   <img id="output2"  src="{{ENV('APP_URL')}}upload_vendor/{{$year}}/{{$month}}/original/{{$creatives->photo_url}}" style="width:100%;height:100%;padding:20px;">
   @elseif($creatives->file_type == 'other' && $creatives->other_document_type != 'ppt')
   <iframe src="{{ENV('APP_URL')}}upload_vendor/{{$year}}/{{$month}}/{{$creatives->photo_url}}" width="400" height="400" autostart="false"></iframe>
   @else
   <a href="{{ENV('APP_URL')}}upload_vendor/{{$year}}/{{$month}}/{{$creatives->photo_url}}">click to view powerpoint show</a>
   @endif
-->
...
<div class="ad_list_image"><img src="{{ENV('APP_URL')}}upload_vendor/{{$year}}/{{$month}}/original/{{$creatives->photo_url}}" id="32" width="100%"></div>
...
<img src="{{ENV('APP_URL')}}upload_vendor/{{$year}}/{{$month}}/original/{{$creatives->photo_url}}" class="imgssss" id="imgsssss" >
...
$img1=$_ENV['APP_URL']."upload_vendor/".@$year."/".@$month."/".@$creatives->photo_url;
...
<!--<img src="{{$thumbnail}}" id="32" width="100%" style="box-shadow: 1px 2px 5px 5px ghostwhite;">  -->
<video  width="300" height="200" controls="controls" preload="metadata">
...
@else
<?php 
$images_path_b="upload_vendor/".$year."/".$month."/".$creatives->photo_url;
...
$imgExt = new Imagick();
$imgExt->readImage('upload_vendor/'.$year.'/'.$month.'/'.$creatives->photo_url.'[0]');
...
<div class="col-lg-12 col-md-12 ad_list_img">
<div id="adlist_popup32" class="adlist_popup32" onclick="myFunction_img_pop32()">
<div class="ad_list_image"><img src="{{$images_path_x}}" id="32" width="100%"></div>
<div class="ad_list_imagepopup">
<span class="ad_list_imagepopup_close">X</span>
<embed src="{{ENV('APP_URL')}}upload_vendor/{{$year}}/{{$month}}/{{$creatives->photo_url}}#toolbar=0" style="height:100%;" width="600"></div>
</div>
</div>
</div>
</div>
...
<a href="#"  onclick="myFunction_sharing1()" style="float:right;"><span class="ad_list_imagepopup_closessss">X</span></a>  
      ...
      success: function(data) {
         ...
         if(ocrid) {
            ...
            if(finalfileid == finalocrid) {
               ...
               if(arrfileid[0] != advertisement_id )
               {
               ...
               if(arrocrid[0] != advertisement_id )
               {
               $("#advertisement_not_match").attr("style","display:block");
               }
...
<script>
   $(document).ready(function () {
   var element = document.getElementById("manage_creative_draft");
   element.classList.add("active");
   });
</script>
@endif
...
function getids()
 {
      ...
      var ids_1=document.getElementById("ids_1").value;
      var ids_2=document.getElementById("ids_2").value;
      var ids_3=document.getElementById("ids_3").value;
      var ids_4=document.getElementById("ids_4").value;
      var ids_5=document.getElementById("ids_5").value;
      var ids_6=document.getElementById("ids_6").value;
      var ids_7=document.getElementById("ids_7").value;
      var ids_8=document.getElementById("ids_8").value;
      ...
      var _token = jQuery('input[name="_token"]').val();
      jQuery.ajax({
      url: "{{ENV('APP_URL')}}insert_creative_main",
      method: "POST",

      data: {id:id,_token: _token,ids_1:ids_1,ids_2:ids_2,ids_3:ids_3,ids_4:ids_4,ids_5:ids_5,ids_6:ids_6,ids_7:ids_7,ids_8:ids_8},
      success: function(result) {
         console.log(result);
         // result = $.parseJSON(result);
     ...
     if(result.success == '200'){
      ...
      window.location.href="{{ENV('APP_URL')}}view-creative-vendor-approved?_token=<?php echo csrf_token();?>&advertisement_id=<?php echo base64_decode($ids_1);?>&vendor_id=<?php echo base64_decode($ids_2);?>&archive_category_id=<?php echo base64_decode($ids_3);?>&department_id=<?php echo base64_decode($ids_4);?>&from_date=<?php echo base64_decode($ids_5);?>&to_date=<?php echo base64_decode($ids_6);?>&document_id=<?php echo base64_decode($ids_7);?>&search_creative=<?php echo base64_decode($ids_8);?>";
      });
      }
      else if(result.success == '201'){
      Swal.fire(
      'Success',
      'Creative Approved successfully <br> Vendor email id is not present in system. Kindly update the same.',
      'success'
      ).then(function() {
      window.location.href="{{ENV('APP_URL')}}view-creative-vendor-approved?_token=<?php echo csrf_token();?>&advertisement_id=<?php echo base64_decode($ids_1);?>&vendor_id=<?php echo base64_decode($ids_2);?>&archive_category_id=<?php echo base64_decode($ids_3);?>&department_id=<?php echo base64_decode($ids_4);?>&from_date=<?php echo base64_decode($ids_5);?>&to_date=<?php echo base64_decode($ids_6);?>&document_id=<?php echo base64_decode($ids_7);?>&search_creative=<?php echo base64_decode($ids_8);?>";
      });
...
function getids1()
 {
      ...
      var _token = jQuery('input[name="_token"]').val();
      jQuery.ajax({
      url: "{{ENV('APP_URL')}}insert_creative_main_share",
      method: "POST",
      data: {id:id,_token: _token,ids_1:ids_1,ids_2:ids_2,ids_3:ids_3,ids_4:ids_4,ids_5:ids_5,ids_6:ids_6,ids_7:ids_7,ids_8:ids_8},
      success: function(result) {
        //  console.log(result);
    ...
    /* else if(result.success == '201'){
      ...
      window.location.href="{{ENV('APP_URL')}}view-creative-vendor-approved";
      });
      }*/
         
    if(result.success == '200' || result.success == '201'){
    
    document.getElementById("sharelink").value=result.sharing_link
