    ...
    display: block;
      ...
      <div class="container">
        ...
        <div class="search_filter_box">
            <h4>Search Approve Creatives</h4>
            <div class="width100">
                <div class="row">
                
                    <div class="col-lg-8">
                    <form class="search" action="view-creative-vendor-approved" method="get" role="form" id="searchForm" >
                              {{ csrf_field() }}  
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6 mb-3">
                            <select id="search_creative" name="search_creative">
                            <option value="">--Select Search Types--</option>
                            <option value="newcreatives">New Creatives</option>
                            <option value="commentedcreatives">Commented Creatives</option>
                            </select>
                            <script type="text/javascript">
                            document.getElementById('search_creative').value="{{$search_creative}}";
                            </script>
                            </div>
                           
                            <div class="col-lg-12 mb-3">
                                <button class="btn btn-success ml-2" type="submit" style="margin:0px !important">Search</button>
                                <a class="btn btn-danger ml-2"  style="margin:0px !important" href="{{ENV('APP_URL')}}view-creative-vendor-approved" role="button">Cancel</a>
                            </div>
        ...
        <div class="col-lg-3 ad_list_img">
         <label><button type="button" id="vefiy_check" class="btn btn-success ml-2"  onclick="getids()" disabled>Approve</button></label>
        </div>
        </div>
        </div>
                ...
                <div id="ad_list{{$creative->id}}" class="row ad_list{{$creative->id}}">
                  ...
                  <div class="ad_list_imagepopup">
                   ...
                   <?php 
                    ...
                    $img1=$_ENV['APP_URL']."upload_vendor/".@$year."/".@$month."/".@$creative->photo_url;
                   ...
                   <?php }
                    ...
                    <div class="col-lg-2 manage_col4 ">
                        <div class=" ">
                            <?php 
                            $advertisement_id= ($advertisement_id) ?  $advertisement_id : 'no';
                            $vendor_id= ($vendor_id) ?  $vendor_id : 'no';
                            $archive_category_id= ($archive_category_id) ?  $archive_category_id : 'no';
                            $department_id= ($department_id) ?  $department_id : 'no';
                            $from_date= ($from_date) ?  $from_date : 'no';
                            $to_date= ($to_date) ?  $to_date : 'no';
                            $document_id= ($document_id) ?  $document_id : 'no';
                            $search_creative= ($search_creative) ?  $search_creative : 'no';
                            
                            $APP_URL=$_ENV['APP_URL']."edit-creative-vendor/".base64_encode($creative->id)."/".base64_encode($advertisement_id)."/".base64_encode($vendor_id)."/".base64_encode($archive_category_id)."/".base64_encode($department_id)."/".base64_encode($from_date)."/".base64_encode($to_date)."/".base64_encode($document_id)."/".base64_encode($search_creative);
                            ?>
...
$search_pre=$str_pre1[0]."?".$str_pre1[1].'&advertisement_id='.$advertisement_id.'&vendor_id='.$vendor_id.'&archive_category_id='.$archive_category_id.'&department_id='.$department_id."&from_date=".$from_date."&to_date=".$to_date."&document_id=".$document_id."&search_creative=".$search_creative;
?>

<?php  
$str_nxt=$creatives->url($creatives->currentPage()+1);
$str_nxt1=explode("?",$str_nxt);
$search_nxt=$str_nxt1[0]."?".$str_nxt1[1].'&advertisement_id='.$advertisement_id.'&vendor_id='.$vendor_id.'&archive_category_id='.$archive_category_id.'&department_id='.$department_id."&from_date=".$from_date."&to_date=".$to_date."&document_id=".$document_id."&search_creative=".$search_creative;
?>
<nav aria-label="Page navigation example">
  <ul class="pagination">
    ...
    @for ($i = 1; $i <= $creatives->lastPage(); $i++)
      ...
      $str=$creatives->url($i);
      $str1=explode("?",$str);
      $search=$str1[0]."?".$str1[1].'&advertisement_id='.$advertisement_id.'&vendor_id='.$vendor_id.'&archive_category_id='.$archive_category_id.'&department_id='.$department_id."&from_date=".$from_date."&to_date=".$to_date."&document_id=".$document_id."&search_creative=".$search_creative;
      ?>
...
async function getids () {
    ...
    jQuery.ajax({
    url: "{{ENV('APP_URL')}}insert_creative_main_bulk",
    type:"POST",
