  ...
  100% {
    ...
    transform: rotate(360deg);
			...
			@endif
                ...
                <!-- START UPLOAD -->
                <form method="POST" action="{{ url('/insert_bluk_upload_before') }}" enctype="multipart/form-data">
                {{ csrf_field() }}

                <div class="row yr-month">
                    <div class="col-lg-6">
                    <a href="{{ENV('APP_URL')}}assets/before/sample_creatives_bulk.csv">Download Sample File To Import</a>
                    </div>
                </div>
...
<nav class="navbar navbar-light bg-light" id="bg-light">
 <!-- <a class="navbar-brand" href="#">-->
  <button type="button" class="btn btn-success" id="vefiy_check" disabled style="border-radius:25px;padding: 8px 30px;" onclick="getids()">Verify</button>
 <!--</a>-->
</nav>
<div class="tableFixHead">
<table class="table table-striped" style="height:30%;font-size: small;" id="header-fixed">
  <thead>
    <tr>
  ...
  @foreach(@$verify as $verified)
    <tr>
      <td scope="row"><input type="checkbox"  onclick="checkboxcheck()" id="ids" name="id" class="from-control checkboxids" value="{{$verified->id}}"></td>
      <td scope="row">{{$id}}</td>
...
<nav class="navbar navbar-light bg-light" id="bg-light">
<a href="{{ENV('APP_URL')}}generate_csv_file_incomplete_before" class="btn btn-info"  id="downloadLink" style="border-radius:25px;padding: 8px 30px;">Export Rejected Data</a>           
<a href="{{ENV('APP_URL')}}bulk_upload_before_clear_all" class="btn btn-info"  style="border-radius:25px;padding: 8px 30px;">Clear All</a></nav>
...
<script>
  ...
  var element = document.getElementById("bluk_upload_before");
...
jQuery.ajax({
url: "{{ENV('APP_URL')}}insert_bulk_creative_main",
method: "POST",
...
$("#downloadLink").click(
    function(e) {   
        ...
        window.location="add-bulk-file-upload-before";
        window.focus();
    }
);
