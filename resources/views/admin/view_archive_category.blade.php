...
<li class="breadcrumb-item active" aria-current="page">Manage Archive Category</li>
</ol>
</nav>

  <h4 class="viewdata"> Manage Archive Category </h4>
    <div class="row">
      <div class="col-12">
        <div class="card">
          <div class="card-header">
            <div class="row">
              ...
              <!-- <div class="col-md-1 offset-5">
                <a href="{{ENV('APP_URL')}}add-archive-category"> <button type="button" class="btn btn-block btn-secondary">Add</button></a>

              </div> -->
            </div>
            <form method="POST" action="{{ url('/insert_archive_category') }}">
            {{ csrf_field() }}
            <div class="row">
              <div class="col-lg-12 ml-4">
              <label class="font-weight-bolder pr-2 add-lable">Name</label> <input type="text" id="name" name="name" placeholder='Name' required>
              <label class="font-weight-bolder pr-2 add-lable">Keyword</label> <input type="text" id="keyword" name="keyword" placeholder='Keyword' required>
              <button class="btn btn-success ml-2" type="submit">Submit</button>
              <a class="btn btn-danger ml-2" href="{{ENV('APP_URL')}}view-archive-category" role="button">Cancel</a>
              </div>
            </div>
          </form>
          </div>
  ...
  var element = document.getElementById("archive_category");
  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      $('#empTable').DataTable({
        ...
        //  ajax: "{{route('archive_category.getarchive_category')}}",
         ajax: "{{ENV('APP_URL')}}archive_category/getarchive_category",
         "language": {
            "infoFiltered":"",
