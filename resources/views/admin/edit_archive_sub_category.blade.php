...
@include('admin/side-menu')
            ...
            <li class="breadcrumb-item"><a href="{{ENV('APP_URL')}}view-archive-sub-category">Manage Archive Sub Category</a></li>
            <li class="breadcrumb-item active" aria-current="page">Edit Archive Sub Category</li>
            </ol>
            </nav>    


            <h4 class="addeditdata"> Edit Archive Sub Category </h4>  
                <!-- START FORM -->
                            ...
                            <div class="wrapper">
                                <div class="container">
                                   @foreach($edit_services as $department)
                                    <form method="POST" action="{{ url('/update_archive_sub_category') }}">
                                    {{ csrf_field() }}
                                    <div class="form-group">
                                       <label for="archive_category_id">Archive Category</label>
                                        ...
                                        <option value="{{$archivecategory->id}}">{{$archivecategory->name}}</option>
                                        @endforeach
                                        </select>
        
                                        <script type="text/javascript">
                                       document.getElementById('archive_category_id').value="{{$department->archive_category_id}}";
                                       </script>
                                        </div>
                                        <div class="form-group">
                                        <label for="name"> Name</label>
                                            <input type="text" class="form-control" placeholder="Name" name="name" id="name" required value="{{$department->name}}">
